<script setup>
const { items , isrecomand} = defineProps(['items', 'isrecomand']);
const linkto = ref("/cardetails/" + items.id);
</script>
<template>
    <div v-if="items"
        class="w-100 min-w-[280px] min-h[450px] mx-auto overflow-hidden bg-white rounded-lg shadow-lg p-2 lg:p-4 flex flex-col gap-3 items-center">
        <h3
            class="w-full text-secondary-500 font-plus-jakarta-sans font-semibold text-lg leading-8 tracking-tighter mb-4 flex items-center justify-between">
        ...
        <div class="relative">
            <img v-if="items.img" :src="items.img" alt="" class="max-h-[120px] h-120px">
        ...
        <div class="w-full flex justify-between items-center">
            <span>
                {{ items.pricePerDay }} / Day
            </span>
            <NuxtLink v-if="linkto" :to="linkto">
                <button type="button"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm p-1 lg:px-5 lg:py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                    Rent Now
                </button>
            </NuxtLink>
        </div>
...
img
: 
"https://dm-assignment-commonshare.koyeb.app/img/11.png"
kindOfTransition
: 
"Automatic"
name
